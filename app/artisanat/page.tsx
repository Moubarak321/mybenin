          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 bg-white p-6 rounded-xl shadow-sm border border-amber-100">
              <div className="relative h-[500px] w-full">
                {/* Carte SVG du Bénin avec points interactifs */}
                <svg
                  width="100%"
                  height="100%"
                  viewBox="0 0 100 100"
                  className="bg-[#F5F0E8] rounded-lg"
                >
                  {/* Contour simplifié du Bénin */}
                  <path
                    d="M20,30 Q25,20 30,25 Q35,15 40,20 Q45,10 50,15 Q55,5 60,10 Q65,0 70,5 L75,15 Q80,20 85,25 L80,35 Q75,40 70,45 L65,55 Q60,60 55,65 L50,75 Q45,80 40,85 L35,75 Q30,70 25,65 L20,55 Q15,50 10,45 L15,35 Z"
                    fill="#FEF5E7"
                    stroke="#8B5A2B"
                    strokeWidth="0.5"
                  />

                  {/* Points interactifs pour chaque région */}
                  {regions.map((region) => (
                    <g key={region.id}>
                      <motion.circle
                        cx={region.position.x}
                        cy={region.position.y}
                        r={hoveredRegion === region.id || selectedRegionId === region.id ? 5 : 3}
                        fill={region.color}
                        stroke="#fff"
                        strokeWidth="0.5"
                      ...
                      />
                      {(hoveredRegion === region.id || selectedRegionId === region.id) && (
                        <text
                          x={region.position.x}
              ...
              <AnimatePresence mode="wait">
                {selectedRegion ? (
                  ...
                  >
                    <Card className="h-full border-none shadow-xl overflow-hidden">
                      ...
                      <CardContent>
                        ...
                        <div className="mt-4">
                          <h4 className="font-semibold text-[#3A2D1E] mb-2">
                  ...
                  <motion.div
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="flex flex-col items-center justify-center h-full text-center bg-white p-6 rounded-xl shadow-sm border border-amber-100"
                  >
                    <MapPin className="h-12 w-12 text-amber-300 mb-4" />
                    <h3 className="text-xl text-[#3A2D1E] mb-2">
                      Explorez la carte artisanale
                    </h3>
                    <p className="text-gray-600">
                      Cliquez sur une région pour découvrir ses trésors artisanaux
                    </p>
                  </motion.div>
                )}
              </AnimatePresence>
